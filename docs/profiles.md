...
This document defines built-in profiles for flutterkill. A profile is a named preset of "safe-to-delete" directories for a given ecosystem.
...
- By default (no `--profiles`), flutterkill uses the `flutter` profile.
...
- `build`: Flutter's build output directory containing compiled binaries for all platforms (APK, IPA, web, desktop). Deleting removes all build artifacts. Regenerated by `flutter build` or `flutter run`.
- `.dart_tool`: Dart's package config, build snapshots, and generated files. Deleting forces re-resolution of packages. Regenerated by `flutter pub get` or `dart pub get`.
- `.fvm`: Flutter Version Management's local SDK cache. Deleting removes the locally pinned Flutter SDK copy. Regenerated by running `fvm install`.
- `ephemeral`: Auto-generated platform runner files for macOS and Windows (`macos/Flutter/ephemeral`, `windows/flutter/ephemeral`). Deleting is safe. Regenerated automatically by `flutter run` or `flutter build`.
- `.gradle`: Gradle's cache and wrapper files within the `android/` subdirectory. Deleting may slow down the next Android build. Regenerated automatically by Gradle.
- `Pods`: CocoaPods dependencies for the iOS runner (`ios/Pods`). Deleting requires re-fetching pods. Regenerated by `cd ios && pod install`.
